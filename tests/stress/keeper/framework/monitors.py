...
from .settings import SAMPLER_FLUSH_EVERY, SAMPLER_ROW_FLUSH_THRESHOLD
from .probes import mntr, lgif, prom_metrics
from .sink import sink_clickhouse
from .prom_parse import parse_prometheus_text

class MetricsSampler:
    def __init__(self, nodes, run_meta, scenario_id, topology, sink_url=None, interval_s=5, stage_prefix="run", run_id="", prom_allow_prefixes=None, ctx=None):
        self.nodes = nodes
        ...
        self.prom_allow_prefixes = tuple(prom_allow_prefixes) if prom_allow_prefixes else None
        self._stop = False
        self._th = None
    ...
    def _snapshot_once(self):
        for n in self.nodes:
            try:
                m = mntr(n); l = lgif(n)
                # add numeric mntr fields into unified timeseries
                try:
            ...
            try:
                p = prom_metrics(n)
                parsed = parse_prometheus_text(p, allow_prefixes=self.prom_allow_prefixes)
                for r in parsed:
                    name = r.get("name", "")
                    try:
    ...
    def _loop(self):
        while not self._stop:
            self._snapshot_once()
            self._snap_count += 1
            if self.sink_url and (self._snap_count % self._flush_every == 0 or
                                   len(self._metrics_ts_rows) > self._row_flush_threshold):
                try:
                    self.flush()
                except Exception:
                    pass
